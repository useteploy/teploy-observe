import{u as i}from"./jsxRuntime-B6N0V2p5.js";import{h as d,y as h}from"./hooks-DSTFl5iC.js";import"./index-BR_xgfH0.js";const y={mode:"app"},p=[{title:"Getting Started",pages:[{slug:"intro",title:"Introduction",content:`# What is Observe?
...
`),t=r[0].split("|").slice(1,-1).map(a=>a.trim()),o=r.slice(2).map(a=>a.split("|").slice(1,-1).map(u=>u.trim()));let c=`<table class="docs-table"><thead><tr>${t.map(a=>`<th>${a}</th>`).join("")}</tr></thead><tbody>`;for(const a of o)c+=`<tr>${a.map(u=>`<td>${u}</td>`).join("")}</tr>`;return c+="</tbody></table>",c}),e=e.replace(/((?:^- .+\n?)+)/gm,s=>"<ul>"+s.trim().split(`
...
`),e}function S(){const[n,e]=d("intro");h(()=>{const t=typeof window<"u"?window.location.hash.slice(1):"";t&&e(t)},[]);const l=p.flatMap(t=>t.pages),s=l.find(t=>t.slug===n)||l[0],r=t=>{e(t),typeof window<"u"&&(window.location.hash=t,window.scrollTo(0,0))};return i("div",{class:"docs-layout",children:[i("aside",{class:"docs-sidebar",children:p.map(t=>i("div",{class:"docs-section",children:[i("div",{class:"docs-section-title",children:t.title}),t.pages.map(o=>i("button",{class:`docs-nav-item ${n===o.slug?"docs-nav-item--active":""}`,onClick:()=>r(o.slug),children:o.title},o.slug))]},t.title))}),i("main",{class:"docs-content",children:i("article",{dangerouslySetInnerHTML:{__html:g(s.content)}})})]})}export{y as config,S as default};
