// The sidebar's nav model, kept out of Sidebar.tsx so it can be asserted
// against without rendering Preact.
//
// A label here is the only word most people will ever read for a whole
// product surface, so it has to name what is behind it rather than describe
// the shape of the page. "Insights" did not: funnels, goal conversions and
// conversion correlations all live at /insights, and someone hunting for a
// funnel scanned twenty-six nav items without recognising any of them. It is
// now "Funnels & Goals".
...
export const NAV_ITEMS: NavItem[] = [
  ...
  // Funnel-shaped icon, so the picture says the same thing the label does.
  { key: "insights", label: "Funnels & Goals", icon: "M3 4h18v2.5L14 13v7l-4 2v-9L3 6.5V4z", href: "/insights" },
...
export function navKeyFor(path: string): string | null {
